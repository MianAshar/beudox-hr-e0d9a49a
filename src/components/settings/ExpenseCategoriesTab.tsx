...
const ExpenseCategoriesTab = () => {
  ...
  useEffect(() => {
    if (!loading && categories.length === 0 && companyId && !seeding) {
      seedDefaults();
    }
    ...
    try {
      for (let ci = 0; ci < FORTE_SEED.length; ci++) {
        ...
        const { data: cat } = await supabase
          .from('expense_categories')
          .insert({ company_id: companyId, name: seed.name, display_order: ci })
          .select()
          .single();
        if (cat && seed.items.length > 0) {
          await supabase.from('expense_line_items').insert(
            seed.items.map((item, ii) => ({
  ...
  const moveCategoryOrder = async (id: string, direction: 'up' | 'down') => {
    ...
    await Promise.all([
      supabase.from('expense_categories').update({ display_order: b.display_order }).eq('id', a.id).eq('company_id', companyId!),
      supabase.from('expense_categories').update({ display_order: a.display_order }).eq('id', b.id).eq('company_id', companyId!),
    ]);
    fetchData();
  };
  ...
  const addLineItem = async () => {
    ...
    const { error } = await supabase.from('expense_line_items').insert({
      company_id: companyId,
      category_id: addingItemForCategory,
  ...
  const saveEditItem = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        is_recurring: editItemRecurring,
  ...
  const moveItemOrder = async (id: string, categoryId: string, direction: 'up' | 'down') => {
    ...
    await Promise.all([
      supabase.from('expense_line_items').update({ display_order: b.display_order }).eq('id', a.id).eq('company_id', companyId!),
      supabase.from('expense_line_items').update({ display_order: a.display_order }).eq('id', b.id).eq('company_id', companyId!),
    ]);
    fetchData();
  };
  ...
  return (
    <div className="space-y-6" style={{ fontFamily: 'var(--ff-body)' }}>
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[16px] font-semibold text-foreground" style={{ fontFamily: 'var(--ff-display)' }}>
            Expense Categories
          </h2>
          <p className="text-[13px] text-muted-foreground mt-0.5">
            Define categories and recurring line items that pre-fill the monthly Finance Sheet.
          </p>
        </div>
      ...
      {showAddCategory && (
        <div className="flex items-center gap-2 p-3 rounded-md border border-border bg-muted/30">
          <Input
            ...
            autoFocus
          />
          <Button size="sm" variant="default" onClick={addCategory} className="h-8 px-3">
            <Check className="h-3.5 w-3.5" />
          </Button>
          <Button size="sm" variant="ghost" onClick={() => { setShowAddCategory(false); setNewCategoryName(''); }} className="h-8 px-3">
        ...
        <div className="text-center py-12 text-muted-foreground text-sm">
          {seeding ? (
            <div className="flex items-center justify-center gap-2">
              <RefreshCw className="h-4 w-4 animate-spin" /> Seeding default categories…
            </div>
          ) : (
      ...
      <Accordion type="multiple" className="space-y-2">
        {categories.map((cat, catIdx) => {
          ...
          return (
            <AccordionItem key={cat.id} value={cat.id} className="border rounded-md px-1">
              <div className="flex items-center gap-1">
                {/* Sort arrows */}
                <div className="flex flex-col">
                  <button
                    onClick={(e) => { e.stopPropagation(); moveCategoryOrder(cat.id, 'up'); }}
                    disabled={catIdx === 0}
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); moveCategoryOrder(cat.id, 'down'); }}
                ...
                {isEditing ? (
                  <div className="flex items-center gap-2 flex-1 py-2">
                    <Input
                      ...
                      autoFocus
                    />
                    <Button size="sm" variant="default" onClick={saveEditCategory} className="h-8 px-3">
                      <Check className="h-3.5 w-3.5" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => setEditingCategoryId(null)} className="h-8 px-3">
                      <X className="h-3.5 w-3.5" />
                    </Button>
                  </div>
                ) : (
                ...
                <div className="space-y-1 pl-6">
                    ...
                    return (
                      <div key={item.id} className="flex items-center gap-2 py-1.5 px-2 rounded hover:bg-muted/50 group">
                        ...
                        <div className="flex flex-col opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => moveItemOrder(item.id, cat.id, 'up')}
                            disabled={itemIdx === 0}
                            className="p-0.5 text-muted-foreground hover:text-foreground disabled:opacity-30"
                          >
                          ...
                          <button
                            onClick={() => moveItemOrder(item.id, cat.id, 'down')}
                        ...
                        {isItemEditing ? (
                          <div className="flex items-center gap-2 flex-1">
                            <Input
                              ...
                              className="max-w-xs text-[13px] h-7"
                              autoFocus
                            />
                            <div className="flex items-center gap-1.5">
                              <Switch
                                checked={editItemRecurring}
                                onCheckedChange={setEditItemRecurring}
                                className="scale-75"
                              />
                              <span className="text-[11px] text-muted-foreground">Recurring</span>
                            </div>
                            <Button size="sm" variant="default" onClick={saveEditItem} className="h-7 px-2">
                              <Check className="h-3 w-3" />
                            </Button>
                            <Button size="sm" variant="ghost" onClick={() => setEditingItemId(null)} className="h-7 px-2">
                              <X className="h-3 w-3" />
                            </Button>
                          </div>
                        ) : (
                            ...
                            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => {
                                  ...
                                  setEditItemRecurring(item.is_recurring);
                                }}
                              ...
                              <button
                                onClick={() => setDeleteTarget({ type: 'item', id: item.id, name: item.description })}
                  ...
                  {addingItemForCategory === cat.id ? (
                    <div className="flex items-center gap-2 py-2 px-2">
                      ...
                      <Button size="sm" variant="default" onClick={addLineItem} className="h-7 px-2">
                        <Check className="h-3 w-3" />
                      </Button>
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => { setAddingItemForCategory(null); setNewItemDescription(''); setNewItemRecurring(true); }}
                        className="h-7 px-2"
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => { setAddingItemForCategory(cat.id); setNewItemDescription(''); setNewItemRecurring(true); }}
                      className="text-[12px] text-muted-foreground hover:text-foreground gap-1 mt-1"
                    >
